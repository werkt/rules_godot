...
def godot_binary(
        ...
        *,
        ...
        export_templates_external = None,
        version = "4.4",
        flavor = "stable",
        repo_name = "godot",
        srcs = [],
    ...
    if not export_templates_external:
        export_templates_external = export_templates
    remap_paths = {
        "external/%s/templates" % export_templates_external: "godot/export_templates/%s.%s" % (version, flavor),
    }
    plugin_names = []
    ...
    for plugin, res in plugins.items():
        ...
        # local references will end up with a leading slash
        if src[0] == '/':
            src = src[1:]
        remap_paths[src] = res
